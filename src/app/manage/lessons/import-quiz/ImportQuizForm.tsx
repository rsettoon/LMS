...
import { importQuizzes, type QuizImportState } from "./actions";

export default function ImportQuizForm() {
  const [state, formAction, pending] = useActionState<
    QuizImportState,
    FormData
  >(importQuizzes, undefined);
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      {state && "lessons" in state && (
        <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-sm dark:border-zinc-800 dark:bg-zinc-800/50">
          <p className="font-medium text-green-700 dark:text-green-400">
            ✅ {state.questions} question{state.questions === 1 ? "" : "s"}{" "}
            imported across {state.lessons} lesson
            {state.lessons === 1 ? "" : "s"}
          </p>
      ...
      >
        {pending ? "Importing…" : "Import quiz questions"}
      </button>
    </form>
  );
}
